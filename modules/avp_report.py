...
from modules.avp_engine import finalize_calculations, parse_report_date
...
def _render_html(data, report_type):
    ...
    if "Edition Date" in data.columns and data["Edition Date"].notna().any():
        issue_dt = parse_report_date(data["Edition Date"].dropna().iloc[0])
        issue_date = issue_dt.strftime("%d %B %Y") if pd.notna(issue_dt) else "—"
    ...
    for _, r in data.iterrows():
        ...
        rows.append(
            f"""
            <tr>
                <td>{parse_report_date(r['Edition Date']).strftime('%d/%m/%Y') if pd.notna(parse_report_date(r['Edition Date'])) else '—'}</td>
