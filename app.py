...
st.set_page_config(page_title="PressIQ Analytics", page_icon="🏭", layout="wide")
...
if area == "Waste Intelligence":
    module = st.sidebar.radio(
        "Select Waste Tool",
        ["Pan India Waste Tracker Analyzer", "ADAM Production Report Analyzer"]
    )
else:
    module = st.sidebar.radio(
        "Select Downtime Tool",
        ["Overall Downtime Analyzer", "0–4 Min Micro Stoppage Analyzer", "Web Break Downtime Analyzer"]
    )
...
def read_tracker_file(uploaded_file):
    ...
    if not summary.empty:
        summary["Waste per Cold Start Kg"] = summary.apply(lambda r: safe_div(r["Cold Waste Kg"], r["Cold Starts"]), axis=1)
        summary["Waste per Warm Start Kg"] = summary.apply(lambda r: safe_div(r["Warm Planned Waste Kg"] + r["Warm Unplanned Waste Kg"], r["Warm Starts"]), axis=1)
        summary["Waste per Unplanned Stop Kg"] = summary.apply(lambda r: safe_div(r["Warm Unplanned Waste Kg"], r["Warm Unplanned Stoppages"]), axis=1)
        summary["Waste per GNP Kg"] = summary.apply(lambda r: safe_div(r["Total Waste Kg"], r["GNP Count"]), axis=1)
        summary["Waste per Extra Folder Kg"] = summary.apply(lambda r: safe_div(r["Total Waste Kg"], r["Extra Folder"]), axis=1)
...
if module == "Pan India Waste Tracker Analyzer":
    ...
    if uploaded_file:
        ...
        with tab2:
            ...
            compare_df = pd.DataFrame({
                "KPI": [
                    ...
                    "Non Print Waste MT",
                    "Cold Waste Kg",
                    "Warm Planned Waste Kg",
                    "Warm Unplanned Waste Kg",
                    "Running Waste Kg",
                    "Reel End Waste Kg",
                    "Tear Off Waste Kg",
                    "Sweep Waste Kg",
                ...
                plant: [
                    ...
                    plant_row["Print Waste MT"],
                    plant_row["Non Print Waste MT"],
                    plant_row["Cold Waste Kg"],
                    plant_row["Warm Planned Waste Kg"],
                    plant_row["Warm Unplanned Waste Kg"],
                    plant_row["Running Waste Kg"],
                    plant_row["Reel End Waste Kg"],
                    plant_row["Tear Off Waste Kg"],
                    plant_row["Sweep Waste Kg"],
                    plant_row["Trial Waste Kg"],
                    plant_row["Cold Starts"],
                    plant_row["Warm Starts"],
                    plant_row["GNP Count"],
                    plant_row["Extra Folder"]
                ],
                "Pan India": [
                    pan_waste_pct,
                    summary["Total Waste MT"].sum(),
                    summary["Print Waste MT"].sum(),
                    summary["Non Print Waste MT"].sum(),
                    summary["Cold Waste Kg"].sum(),
                    summary["Warm Planned Waste Kg"].sum(),
                    summary["Warm Unplanned Waste Kg"].sum(),
                    summary["Running Waste Kg"].sum(),
                    summary["Reel End Waste Kg"].sum(),
                    summary["Tear Off Waste Kg"].sum(),
                    summary["Sweep Waste Kg"].sum(),
                    summary["Trial Waste Kg"].sum(),
                    summary["Cold Starts"].sum(),
                    summary["Warm Starts"].sum(),
                    summary["GNP Count"].sum(),
                    summary["Extra Folder"].sum()
                ]
            })
            ...
            if plant_row["Waste %"] > pan_waste_pct:
                ...
                insight_box(
                    f"<b>{plant} is above Pan India average.</b> Approx saving opportunity if it reaches Pan India average: "
                    f"{gap_waste/1000:.1f} MT.", "warning"
                )
            else:
        ...
        with tab3:
            ...
            comp = pd.DataFrame({
                "KPI": [
                    ...
                    "Cold Starts",
                    "Warm Starts",
                    "Warm Unplanned Stoppages",
                    "GNP Count",
                    "Extra Folder",
                    "Waste per Cold Start Kg",
                    "Waste per Warm Start Kg",
                    "Waste per Unplanned Stop Kg",
                    "Waste per GNP Kg",
        ...
        with tab5:
            ...
            outliers = daily_all[
                (daily_all["Waste %"] > pan_waste_pct) & (daily_all["Consumption Kg"] > 0)
...
# ---------------- PLACEHOLDERS ----------------
    ...
    st.info("ADAM Production Report Analyzer will be added next.")

elif module == "Overall Downtime Analyzer":
    st.info("Overall Downtime Analyzer will be restored after Waste Tracker module is stabilized.")

elif module == "0–4 Min Micro Stoppage Analyzer":
    st.info("0–4 Min Micro Stoppage Analyzer will be restored after Waste Tracker module is stabilized.")
